...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // minimise the use of actuators i.e. 
    ...
    //minimise the value gap between sequential actuations
    ...
    for (int t = 1; t < N; t++) {
      ...
      // simulate latency by using previous actuations 
      if (t > 1) {
        a0 = vars[a_start + t - 2];
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  size_t n_vars = n_constraints + n_actuators * (N-1);
  ...
  //return the actuator (steering, throttle)  solution components
  result.push_back(solution.x[delta_start]);
  result.push_back(solution.x[a_start]);

  //also return the x,y  solution components
  for (int i = 0; i < N-1; i++) {
